...
Runtime parsing uses html5gum's html5ever adapter and a native node arena. This
combination preserves duplicate HTML attributes in the qualified cases.
...
The fixture contains 44,012 entries. One entry can assert multiple intermediate
...
| pagination | 1,995 | Both algorithms on identical HTML and URL |
...
| public | 168 | Saved page, public pagination/skip options and URL formatting |
...
The fixture [testdata/go-reference.json](testdata/go-reference.json) is
39,819,064 bytes. SHA-256:
`4ae82b0de7db0c6459a138d8dbb3d139951bb3294d1318e5220c86b46fffc6e4`.
It was regenerated independently and matched byte-for-byte. Default tests use
this immutable snapshot; the opt-in live test regenerates it using Go.
...
zero-byte reader ties in this fixture decode identically. This does not establish
universal output equivalence for genuinely ambiguous byte streams.

Go's page-number detector iterates candidate maps. Rust's stable key ordering
may choose a different equal-ranked candidate. Current fixtures reproduce exactly,
but ambiguous map-order cases are not an approved compatibility waiver.

The parser and Unicode libraries have their own versions. Not all malformed HTML,
foreign-element namespaces, invalid UTF-8 URL paths, newly assigned Unicode
characters, or extreme nesting are qualified. The mutable node arena assumes
valid acyclic indices. Extracted HTML is not a security sanitizer.
...
separate milestone. The corpus benchmark below adds measured evidence and records
its remaining differences; no release or publication is implied.
...
count, serialized result-node HTML, next-page URL, and previous-page URL. Text,
title, and word count agree on all 983 pages for all three settings. All six fields
agree on 982/983 pages with pagination skipped or numbered and 981/983 with
previous/next detection. This benchmark does not compare metadata or image lists;
those fields have separate coverage in the offline differential fixture.

Two known exceptions remain; neither is waived as equivalent:

- `harrypotter.fandom.com.spell.html`: HTML differs in all settings. Rust removes
  an SVG `xlink:href` attribute that Go retains, and figure whitespace differs.
  Text, title, and word count are identical. This is within the previously
  unqualified namespace/parser surface, not proof of general HTML equivalence.
- `archive.modabot.de.serkan.html`: in previous/next mode, Go returns no previous
  page while Rust returns
  `https://web.archive.org/web/20190717140047/http://www.modabot.de`.
  The cause is not yet established. Other compared fields agree.

### Timing Method

The [README](README.md#quality-and-speed) contains the quality and speed tables
comparing Go with the final Rust implementation, both rebuilt for this run.
...
[testdata/benchmark-final-results.json](testdata/benchmark-final-results.json).
...
Observed min-max times per 983-page pass were 4,050-5,171 ms Go and 1,362-1,701 ms
Rust with pagination skipped; 5,066-5,900 ms Go and 1,830-2,171 ms Rust for
previous/next; and 4,311-4,925 ms Go and 1,493-2,204 ms Rust for numbered pages.
...
These run offline along with the 44,012 Go-generated differential entries. This
...
Local verification on 2026-09-14, with Rust 1.98.1 and Go 1.27.1:

| Gate | Observed Result |
| --- | --- |
| Linux x86_64 / WSL debug, locked/offline | 21 tests and one README doctest passed; live Go check is opt-in |
| Linux x86_64 / WSL release, locked/offline | Same 21 tests and one doctest passed |
| Native Windows x86_64 / GNU release | All 22 tests, including fresh Go parity, and one doctest passed |
| Linux formatting / Clippy / release build | Pinned rustfmt, all-target `-D warnings`, and build passed |
| Go oracle | Build, vet, module verification, and `go mod tidy -diff` passed |
| Independent fixture and table generation | Exact SHA-256 matches; live fixture also compared byte-for-byte |
| Source notice and page hashes | Retained notices and all public HTML inputs verified |
| `cargo package --locked --offline --allow-dirty` | Package built and verified, including corpus and notices |
| Extracted-package release tests | 21 offline tests and one doctest passed in an isolated target directory |
| Independent packaged consumer | HTML/reader extraction, title, 1,172 words, repeated calls, original tree unchanged |
| Absolute `GO` executable without Go on PATH | Live fixture and encoding-table exports passed |

The package contains the complete offline test corpus and reference tooling;
compressed size was approximately 1.1 MiB. Dependencies were cached for offline
checks, but package and consumer sources were rebuilt independently. The CI
configuration covers Linux and Windows/MSVC; hosted CI, native MSVC, macOS,
ARM64, and older Rust versions have not been executed locally. There are no
published artifacts for this port. The later corpus measurements and additional
upstream title assertions are described above; this table records the initial
port qualification rather than claiming those later additions ran in every
previous platform check.